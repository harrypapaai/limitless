...
pub(crate) async fn deposit_limitless_liquidity(args: &ArgMatches) {
    ...
    let ix = limitless::client::instructions::deposit_liquidity_ix(
        &payer_signer.pubkey(),
        &token_0_mint,
        &token_1_mint,
        &payer_token_0_ata,
    ...
    let signature = rpc_client.send_and_confirm_transaction(&tx).await.expect("could deposit limitless liquidity");
    println!("Signature {}", signature.to_string());
}


pub(crate) fn cmd() -> Command<'static> {
    Command::new("deposit-limitless-liquidity").
        ...
        arg(
            Arg::new("lp-token-amt").
                long("lp-token-amt").
                help("amount of lp token to mint and deposit").
                takes_value(true).
                required(true)
        )
}